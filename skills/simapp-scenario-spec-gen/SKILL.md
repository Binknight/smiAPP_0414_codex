...
- 当前场景目录下的 `apps/scenarios/<app_type>/scenarioxxx/spec/`，其中 `<app_type>` 为 APP 类型（如 `travel`、`explore`、`generic` 等）

默认命名规则：

- 如果当前目标目录是 `apps/scenarios/travel/scenario001/spec/` 且输入文件是 `scenario1.json`，则输出为 `apps/scenarios/travel/scenario001/spec/scenario1.spec.md`
- 如果当前目标目录是 `apps/scenarios/travel/scenario001/spec/` 且输入文件是 `foo/bar/case-a.json`，则输出为 `apps/scenarios/travel/scenario001/spec/case-a.spec.md`
